...
class CBSState:
    ...
    def compute_cost(self): # need to implement
        """
        ...
        self.set_cost(totalCost)
            
        return totalCost
        
    
    def is_solution(self): # need to implment
        """
        ...
        for path in self._paths.values():
            for node in path:
                location = (node, node.get_g())
                if location not in vistedStates:
                    vistedStates.add(location)
                else:
    ...
    def successors(self): # need to implment
        """
        ...
        conflictNode = conflict[0]
        conflictTime = conflict[1]
        ...
        for path in self._paths.values():
            for node in path:
               if node == conflictNode and node.get_g() == conflictTime:
                   conflictAgents.append(agentIdentifier)
            agentIdentifier += 1

        
        for agent in conflictAgents:
            c = CBSState(self._map, self._starts, self._goals)
            c._constraints = copy.deepcopy(self._constraints)
            c.set_constraint(conflictNode, conflictTime, agent)
            successors.append(c)
...
class CBS():
    def search(self, start): # need to implment
        """
        Performs CBS search for the problem defined in start.
        """
        openList = []
        cost = start.compute_cost()
        ...
        while len(openList) > 0:
            ...
            if n.is_solution()[0] == True:
                return n.get_paths(), n._cost
            children = n.successors()
            for child in children:
                childCost = child.compute_cost()
                if childCost != -1:
